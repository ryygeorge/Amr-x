...
// Add to pharma-supabase.js after existing functions

async function fetchMLPredictions(pharmacist) {
  try {
    if (!pharmacist || !pharmacist.district) return [];
    
    // Fetch ML predictions for pharmacist's district
    const { data: predictions, error } = await supabase
      .from('ml_predictions')
      .select('*')
      .eq('district', pharmacist.district.toLowerCase())
      .order('created_at', { ascending: false })
      .limit(10);
    
    if (error) {
      console.warn('Error fetching ML predictions:', error);
      return [];
    }
...
// Update the initRealtime function to include ML data
// Inside initRealtime(), after pharmacist is fetched:
const mlPredictions = await fetchMLPredictions(pharmacist);
